...
import { ChickenOptions } from '../../types/characters';
...
export function drawMrsChicken(
  ...
  const baseSquash = options.squash ?? animState?.squash ?? 1.0;
  ...
  const flap = options.flap ?? animState?.armWave ?? 0;
  const squawk = options.squawk ?? animState?.squawk ?? 0;
  const eyeBlink = options.eyeBlink ?? animState?.isBlinking ?? false;
  const facingLeft = options.facingLeft ?? animState?.facingLeft ?? false;
  const headBob = options.headBob ?? (animState?.wobbleAngle ? Math.sin(animState.wobbleTimer * 4) * 2.5 : 0);
